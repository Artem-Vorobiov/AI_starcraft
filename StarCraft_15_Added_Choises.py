...
class SentdeBot(sc2.BotAI):
    ...
    #   Commands begings here
    async def on_step(self, iteration):         

        self.time = (self.state.game_loop/22.4) / 60
        #print('Time:',self.time)
        await self.distribute_workers()     #   Without AI
        await self.scout()                  #   Without AI
        await self.intel()                  #   Withoit AI -> Greate and Display Map that further will be analyzed (OpenCV)

        #   Jump to line 375 and go ... 
        await self.do_something()           #   AI makes decision
...
#   IT'S MAIN VERY NEW BLOCK that leads by AI
#   In this script Should be concentrated RIGHT HERE
#                   START OFF   
    ...
    async def do_nothing(self):                         #   4. We here, execute
        wait = random.randrange(7, 100)/100             #   5. Got wait = 0.07 to 1
        self.do_something_after = self.time + wait      #   6. do_something_after =  1 + 0.07
    ...
    async def do_something(self):
        # Pretend we aren't useing MODEL. Just randon choise

        if self.time > self.do_something_after:         
        #   1. We here
        #   7. Again waiting a bit
            if self.use_model:
