...
class SentdeBot(sc2.BotAI):
 ...
 CYBERNETICSCORE, STARGATE, VOIDRAY'''
    ...
    async def expand(self):
        try:
            ...
            if self.units(NEXUS).amount < self.time/2 and self.can_afford(NEXUS):
                # print('\n\t Expand Method --> Amount:', self.units(NEXUS).amount)       #   Expand Method --> Amount: 1
                # print('\n\t Expand Method --> Amount:', type(self.units(NEXUS).amount)) #   <class 'int'>
    ...
    async def attack(self):
        ...
        if len(self.units(VOIDRAY).idle) > 3:
            ...
            if self.time > self.do_something_after:
                if self.use_model:
                    ...
                    print('\n\t prediction', prediction)            #   [[0.2657684  0.23639433 0.23271695 0.26512033]]
                    print('\n\t prediction', type(prediction))      #   <class 'numpy.ndarray'>
                    choice = np.argmax(prediction[0])
                    print('\n\t prediction', choice)                #   0
                    print('\n\t prediction', type(choice))          #   <class 'numpy.int64'>
                else:
